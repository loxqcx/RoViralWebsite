...
New review embeds use simple sequential IDs such as `review1`, `review2`, and `review3`. Use `/deleter id:review1` to delete one review in any moderation state. Use `/deleteallrev` to delete every bot-authored review embed while leaving unrelated channel messages alone. Older UUID review IDs remain compatible with `/deleter`. Both commands confirm privately and permit configured moderators, members with Manage Messages, or staff who can access the private review channel. The website updates on its next refresh.
