...
New review embeds use simple sequential IDs such as `review1`, `review2`, and `review3`. Use `/deleter id:review1` to delete one review in any moderation state. Use `/deleteallrev` to mark every bot-authored review embed as `Deleted` while leaving all channel messages and unrelated embeds in place. The bulk command publicly confirms `All reviews have been deleted`; `/deleter` still confirms privately. Older UUID review IDs remain compatible with `/deleter`. Both commands permit configured moderators, members with Manage Messages, or staff who can access the private review channel. The website updates on its next refresh.
