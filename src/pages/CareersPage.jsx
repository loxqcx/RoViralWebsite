...
export default function CareersPage() {
  const subject = encodeURIComponent('RoViral application');
  return (
    <>
        ...
        <div className="container">
          {siteConfig.careers.map((role, index) => (
            <article className="career-row" key={role.title}>
              ...
              <div><small>{role.type}</small><a href={`mailto:${siteConfig.brand.email}?subject=${subject}%20-%20${encodeURIComponent(role.title)}`}>Apply <ArrowUpRight size={17} /></a></div>
            </article>
          ))}
          <div className="career-note"><strong>Don’t see your exact role?</strong><p>Send a tight intro and your best work. We keep a short list of people worth calling when the right project arrives.</p></div>
        </div>
      </section>
    </>
  );
